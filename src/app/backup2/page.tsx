...
export default function Backup2Page() {
  ...
  return (
    <section className="page">
      <div className="page-head">
        ...
        <EditableDesc k="backup2-desc" def="로그형(웹툰 스크롤) / 단일형(좌우 넘김) · 리스트/갤러리 보기 전환" />
      </div>
      <div className="toolrow">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {user && <button className="btn btn-dark" onClick={() => router.push('/backup2/write')}>✎ WRITE</button>}
        </div>
      </div>
          ...
          {visible.map((p, i) => {
            ...
            return (
              <div key={p.id} className="panel g-item" {...sort(i)}
                onClick={() => { if (!folded && !editOn) router.push(`/backup2/${p.id}`); }}>
      ...
      <div className="panel flush" style={{ display: view === 'list' && visible.length > 0 ? undefined : 'none' }}>
          {visible.map(p => (
            <div key={p.id} className="list-item" onClick={() => router.push(`/backup2/${p.id}`)}>
