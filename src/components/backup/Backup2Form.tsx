...
export function Backup2Form({ initial }: { initial: BackupPost | null }) {
  const router = useRouter();
  const { user } = useAuth();
  const toast = useToast();
  const [posts, setPosts] = useLocalList<BackupPost>('ohome.backup2.v1', BACKUP_SEED);
  const isNew = !initial;
  ...
  const post = async () => {
    ...
    if (isNew) {
      ...
      toast('저장되었습니다');
      router.push(`/backup2/${initial.id}`);
    }
  };

  return (
    <section className="page">
      <div className="page-head"><h1>{isNew ? 'WRITE' : 'EDIT'}</h1><EditableDesc k={isNew ? 'backup2-write-desc' : 'backup2-edit-desc'} def={isNew ? '그림백업 글 작성' : '그림백업 글 수정'} /></div>
        ...
        <div>
          ...
          <div className="form-actions">
            <button className="btn btn-onbk"
              onClick={() => router.push(isNew ? '/backup2' : `/backup2/${initial.id}`)}>CANCEL</button>
